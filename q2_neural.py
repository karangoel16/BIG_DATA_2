...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    layer12 = sigmoid(layer1)#this is the activation function after first layer
    layer2   = np.dot(layer12, W2) + b2
    # need to calculate the softmax loss
    probs = softmax(layer2)
    cost  = - np.sum(labels*np.log(probs))#this is the final cost calculated for the difference in the output
    ### END YOUR CODE
    ...
    gradW2    = np.dot(layer12.T, dx)
    gradb2    = np.sum(dx, axis=0)
    dlayer2   = np.dot(dx, W2.T)
    dlayer1   = sigmoid_grad(layer12) * dlayer2
